...
function Features({ features }: featu) {
    ...
    return (
        ...
        <div className='p-4 md:p-6 xl:p-10 mt-7  bg-white rounded-xl border shadow-md '>
            <h2 className='text-xl md:text-2xl font-semibold'>Features</h2>
            <div
                className={"grid gap-4 mt-4 grid-cols-1 sm:grid-cols-2  xl:grid-cols-4 sm:max-h-none max-h-[400px] overflow-y-auto"}
            >
                {Object.entries(features).map(([features, value]) => (
                    <div key={features} className='flex  items-center gap-2'>
                        ...
                        <h2 className='text-sm break-all'> {features.toUpperCase()}</h2>
                    </div>
                ))}
            </div>
